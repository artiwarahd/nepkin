...
class App extends React.Component<ExcalidrawProps, AppState> {
  ...
  private addTextOnStickyNote(text: any, x: number, y: number, parentId: any) {
    const element = newTextElement({
      x: x,
      y: y,
      strokeColor: this.state.currentItemStrokeColor,
    ...
    this.setState({
      selectedElementIds: { [element.id]: true, [parentId]: true },
    });

    setTimeout(() => {
      this.actionManager.executeAction(actionGroup);
    });
  ...
  private onPointerUpFromPointerDownHandler(
    ...
    return withBatchedUpdates((childEvent: PointerEvent) => {
      ...
      if (draggingElement.type === "sticky-note") {
        const x = draggingElement.x + 20;
        const y = draggingElement.y + 20;

        this.addTextOnStickyNote("note", x, y, draggingElement.id);
      }
